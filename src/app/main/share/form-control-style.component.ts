...
@Component({
  ...
  entryComponents: [ControlMessageComponent],
})
export class FormControlStyleComponent
  ...
  ngAfterViewInit(): void {
    ...
    if (this.formComponent !== "") {
      this.inputElement = this.el.nativeElement.querySelector(
        this.formComponent
      );
    } else {
    ...
    if (!componentName) {
      console.error(
        "FormControlStyleComponent: Unable to get the control name. Is the formControlName attribute set correctly?"
      );
      return;
    }
    ...
    if (!(control instanceof FormControl)) {
      console.error(
        `FormControlStyleComponent: Unable to get the FormControl from the form and the control name: ${componentName}.`
      );
      return;
    }
    ...
    this.component.statusChanges.subscribe((status) => {
      if (
        isInputMask !== undefined &&
        isInputMask !== null &&
        this.component.touched
      )
